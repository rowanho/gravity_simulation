...
void updateBody() {
  ...
  for (int i=0; i<NumberOfBodies; i++){      
    for (int j=i+1; j<NumberOfBodies; j++) {
      // Manually need to calculate force + ditance
      ...
      f0 = (x[j][0]-x[i][0]) * massSquaredDist ;
      f1 = (x[j][1]-x[i][1]) * massSquaredDist ;
      f2 = (x[j][2]-x[i][2]) * massSquaredDist ;
  ...
  for (int i=0; i<NumberOfBodies; i++) {
    for (int j=i+1; j<NumberOfBodies; j++) {
      ...
      if ( distance < 0.01) {
        ...
        // The values of object j can now be discarded, and we can safely swap them with those at the end of the array
        for (int k = 0; k < 3; k++){
